...
```bash
python3 preprocess.py SMAP MSL SWaT WADI MSDS SMD MSDS UCR MBA
```
...
```
where `<model>` can be either of 'TranAD', 'GDN', 'MAD_GAN', 'MTAD_GAT', 'MSCRED', 'USAD', 'OmniAnomaly', 'LSTM_AD', and dataset can be one of 'SMAP', 'MSL', 'SWaT', 'WADI', 'SMD', 'MSDS', 'MBA' and 'UCR'. To train with 20% data, use the following command 
```bash
